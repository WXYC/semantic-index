...
3. **Validate and swap.** The conductor downloads the artifact and runs the **fail-closed** validation gate (`scripts/validate_graph_db.py`: SQLite header check + `artist`-count floor + enrichment row counts compared against the seed — a build that started from an empty database and lost enrichment is rejected and **not** swapped). It then clears stale `-wal`/`-shm` files and `os.replace`s the artifact into the live path. The API needs no restart: its per-request read-only open (`api/database.py`) picks up the new file on the next request.
