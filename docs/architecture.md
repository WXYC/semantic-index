...
7. **Export.** Everything is written to the SQLite [graph database](glossary.md#graph-database) (`sqlite_export.py`), including play-level [facet](glossary.md#facet) tables for on-demand filtered PMI (`facet_export.py`). A [GEXF](glossary.md#gexf) file for Gephi can be exported too (`graph_export.py`).
